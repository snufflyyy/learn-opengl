...
Shader shader_create(const char* vertex_shader_source_path, const char* fragment_shader_source_path) {
    ...
    int shader_program_link_result;
    glGetProgramiv(shader.program, GL_LINK_STATUS, &shader_program_link_result);
    if (shader_program_link_result == GL_FALSE) {
        char info_log[512];
...
void shader_set_float_uniform(GLuint uniform_location, float value) {
